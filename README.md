...
`typoglycemia.py` provides the unfiltered `Typoglycemia` baseline and shuffles
valid alphabetic candidate words. `pos_filter.py` adds
`PosFilteredTypoglycemia`, which inherits that baseline and only shuffles
candidate words tagged as nouns or verbs (`NN*` or `VB*` POS tags). The POS
filter accepts a custom `pos_tagger` callable for stricter tagging. When none is
supplied, it uses NLTK if available and otherwise falls back to a conservative
built-in tagger.
