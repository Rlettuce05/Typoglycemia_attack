...
class Typoglycemia:
    """
    Add typoglycemia poisoning to texts in the dataframe.
    """

    def __init__(self, seed, tokenizer=None):
        # dataframe to store all words in the text, their indices dictionary(text_index, index), DF, and shuffled words
    ...
    def load_data_frame(self, data_frame):
        ...
        if self.data_frame is not None:
            raise ValueError(
                "data_frame is already loaded, please create a new Typoglycemia instance to load a new data frame"
            )
    ...
    def gen_poisoned_text(self, max_changed_words=2, text_column="text", image_column="image_path"):
        ...
        for text_index, row in poisoned_texts_df.iterrows():
            ...
            selected_token_indices = np.random.choice(
                len(candidate_tokens),
                size=num_words_to_change,
                replace=False,
                p=df_probs,
            )
            ...
            # if the original text ends with a period, add a period to the end of the poisoned text if needed
    ...
    def _add_word_occurrence(self, word, text_index, index):
        ...
        if not self.all_words_in_text_dict["word"].isin([word]).any():
            word_df = pd.DataFrame(
                {"word": [word], "text_index": [{text_index: [index]}], "DF": [0], "shuffled_word": [word]}
            )
    ...
    def _allowed_word_tokens(self, text):
        """
        Return (token_index, normalized_word) for alphabetic candidate words.
        """
        text = str(text)
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=DATA_PATH,
        help="Path to the file containing image and text list for poisoning",
    )
    parser.add_argument(
        "--output_csv_folder",
        "-oc",
        type=str,
        default="~/data/code/mscoco/poisoned",
        help="Path to save poisoned texts",
    )
...
def load_data_df(data_list_path):
    ...
    if data_list_path.endswith(".csv"):
        return pd.read_csv(data_list_path, header=0, encoding="utf-8")
    if data_list_path.endswith(".tsv"):
        return pd.read_csv(data_list_path, header=0, encoding="utf-8", sep="\t")
    raise ValueError("data_list_path must end with .csv or .tsv")


def run_typoglycemia(typoglycemia_class, argv=None):
    ...
    seed = 42
    typoglycemia = typoglycemia_class(seed)
    ...
    for i in range(20):
        ...
        df_out = typoglycemia.gen_poisoned_text(
            max_changed_words=i + 1,
            text_column="Caption",
...
def main(argv=None):
    return run_typoglycemia(Typoglycemia, argv=argv)
